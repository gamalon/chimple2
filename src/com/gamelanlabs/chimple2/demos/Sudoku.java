...
public class Sudoku extends Demo {
	...
	public Object run(Object ... args) {
		...
		case MODE_BLOCK_PERMUTATION:
			for(int i = 0; i < 9; i++) {
				...
				int[] permutation = chimpPermutation("block"+i, 9-num_givens[i]);
		...
		case MODE_ROW_PERMUTATION:
			for(int i = 0; i < 9; i++) {
				...
				// Generate a permutation of the other digits for this row
				int[] permutation = chimpPermutation("row"+i, 9-num_givens[i]);
		...
		for(int i = 0; i < 9; i++) {
			...
			if(mode != MODE_BLOCK_PERMUTATION) {
				...
				for(int j = 0; j < 3; j++) {
					for(int k = 0; k < 3; k++) {
						house[3*j+k] = cells[3*blockrow + j][3*blockcol + k];
					}
				}
				cost += checkHouse(house);
			}
	 ...
	 */
	public static int checkHouse(int[] house) {
		int missing = 0;
		for(int i = 1; i <= 9; i++) {
